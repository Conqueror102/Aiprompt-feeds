...
export default function AIAgentChat({ agentName, initialPrompt, onClose }: AIAgentChatProps) {
  ...
  const getAgentTheme = (agent: string) => {
    const themes: { [key: string]: { bg: string; accent: string; logo: string } } = {
      ...
      v0: { bg: "bg-slate-50 dark:bg-slate-900/20", accent: "text-slate-600", logo: "⚡" },
      Lovable: { bg: "bg-rose-50 dark:bg-rose-900/20", accent: "text-rose-600", logo: "💖" },
      Bolt: { bg: "bg-yellow-50 dark:bg-yellow-900/20", accent: "text-yellow-600", logo: "⚡" },
      Replit: { bg: "bg-emerald-50 dark:bg-emerald-900/20", accent: "text-emerald-600", logo: "🔧" },
